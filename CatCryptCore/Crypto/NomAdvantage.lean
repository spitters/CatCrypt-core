...
theorem runPkg_par_right (X Y : CatCrypt.Deep.DeepPackage)
    (hsep : CatCrypt.Deep.DeepPackage.sep X Y)
    (h_main : (0, Unit, Bool) ∈ Y.exports.ops)
    (h_notX : (0, Unit, Bool) ∉ X.exports.ops) :
    runPkg (CatCrypt.Deep.DeepPackage.par X Y hsep) = runPkg Y := by
  have h_app : (0, Unit, Bool) ∈ (CatCrypt.Deep.DeepPackage.par X Y hsep).exports.ops := by
    simp only [CatCrypt.Deep.DeepPackage.par]
    exact List.mem_append_right _ h_main
  simp only [runPkg, dif_pos h_app, dif_pos h_main]
  rw [CatCrypt.Deep.DeepPackage.par_impl_code_eq_right X Y hsep 0 Unit Bool h_notX h_main h_app ()]
...
theorem runPkg_interchange_right (p₁ p₂ p₃ p₄ : CatCrypt.Deep.DeepPackage)
    (h₁₂ : CatCrypt.Deep.DeepPackage.sep p₁ p₂)
    (h₃₄ : CatCrypt.Deep.DeepPackage.sep p₃ p₄)
    (h_link_sep : CatCrypt.Deep.DeepPackage.sep
      (CatCrypt.Deep.DeepPackage.link p₁ p₃)
      (CatCrypt.Deep.DeepPackage.link p₂ p₄))
    (h_notMain : (0, Unit, Bool) ∉ p₁.exports.ops)
    (h_main : (0, Unit, Bool) ∈ p₂.exports.ops)
    (h_env : ∀ op dom codom x,
      (CatCrypt.Deep.DeepPackage.linkEnv
        (CatCrypt.Deep.DeepPackage.par p₃ p₄ h₃₄) op dom codom x).eval =
  ...
  simp only [h_app, dite_true]
  simp only [dif_neg h_notMain, ValidCodeBundle.mono]
  rw [CatCrypt.Deep.eval_substOracle, CatCrypt.Deep.eval_substOracle]
  apply CatCrypt.Deep.evalWith_congr
  exact h_env
...
theorem DeepNomAdvantage_par
    (C G₀ G₁ A_C A_G : CatCrypt.Deep.NomPackage)
    (hAsep : CatCrypt.Deep.NomPackage.atomSep A_C A_G)
    (hregA : A_C.registry = A_G.registry)
    (hGsep₀ : CatCrypt.Deep.NomPackage.atomSep C G₀)
    (hregG₀ : C.registry = G₀.registry)
    (hGsep₁ : CatCrypt.Deep.NomPackage.atomSep C G₁)
    (hregG₁ : C.registry = G₁.registry)
    (h_notMain : (0, Unit, Bool) ∉ A_C.pkg.exports.ops)
    (h_main : (0, Unit, Bool) ∈ A_G.pkg.exports.ops)
    (h_link_sep₀ : CatCrypt.Deep.DeepPackage.sep
      (CatCrypt.Deep.DeepPackage.link A_C.pkg C.pkg)
      (CatCrypt.Deep.DeepPackage.link A_G.pkg G₀.pkg))
    (h_link_sep₁ : CatCrypt.Deep.DeepPackage.sep
      (CatCrypt.Deep.DeepPackage.link A_C.pkg C.pkg)
      (CatCrypt.Deep.DeepPackage.link A_G.pkg G₁.pkg))
    (h_env₀ : ∀ op dom codom x,
      (CatCrypt.Deep.DeepPackage.linkEnv
        (CatCrypt.Deep.DeepPackage.par C.pkg G₀.pkg
          (CatCrypt.Deep.NomPackage.sep_of_atomSep C G₀ hGsep₀ hregG₀))
        op dom codom x).eval =
      (CatCrypt.Deep.DeepPackage.linkEnv G₀.pkg op dom codom x).eval)
    (h_env₁ : ∀ op dom codom x,
      (CatCrypt.Deep.DeepPackage.linkEnv
        (CatCrypt.Deep.DeepPackage.par C.pkg G₁.pkg
          (CatCrypt.Deep.NomPackage.sep_of_atomSep C G₁ hGsep₁ hregG₁))
      ...
      (CatCrypt.Deep.DeepPackage.linkEnv G₁.pkg op dom codom x).eval) :
    DeepNomAdvantage
      (CatCrypt.Deep.NomPackage.par C G₀ hGsep₀ hregG₀)
      (CatCrypt.Deep.NomPackage.par C G₁ hGsep₁ hregG₁)
      (CatCrypt.Deep.NomPackage.par A_C A_G hAsep hregA) =
    DeepNomAdvantage G₀ G₁ A_G := by
  simp only [DeepNomAdvantage, CatCrypt.Deep.NomPackage.par]
  congr 1
  · rw [runPkg_interchange_right A_C.pkg A_G.pkg C.pkg G₀.pkg _ _
        h_link_sep₀ h_notMain h_main h_env₀]
    exact runPkg_par_right _ _ h_link_sep₀ h_main h_notMain
  · rw [runPkg_interchange_right A_C.pkg A_G.pkg C.pkg G₁.pkg _ _
        h_link_sep₁ h_notMain h_main h_env₁]
    exact runPkg_par_right _ _ h_link_sep₁ h_main h_notMain
