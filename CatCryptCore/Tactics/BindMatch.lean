...
macro "ssprove_swap_at" n:num
    c₁:term:max c₂:term:max hc₁:term:max hc₂:term:max : tactic => do
  let depth := n.getNat
  let mut steps : Array (Lean.TSyntax `conv) := #[]
  for _ in [0:depth] do
    steps := steps.push (← `(conv| arg 2))
    steps := steps.push (← `(conv| ext _))
  steps := steps.push (← `(conv| rw [SPComp.isPure_bind_comm $c₁ $c₂ _ $hc₁ $hc₂]))
  `(tactic| conv_lhs => $[$steps]*)
