...
When proving `p₀ ≈₀ p₁` (perfect indistinguishability), we choose an invariant
and prove that all operations preserve it while establishing equal behavior.
...
In the original CatCrypt/Rocq, this is handled via `eq_rel_perf_ind_eq` which
uses heap equality as a special case that doesn't go through the locality framework.
