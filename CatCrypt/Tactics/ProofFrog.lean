...
/-- `ssprove_auto` applies all ProofFrog-style simplifications automatically.
...
```
-/
macro "ssprove_auto" : tactic => `(tactic| (
...
/-- `ssprove_auto!` is a more aggressive version that also tries sync rules.
