...
theorem qFork_success_ge [Fintype R] [Nonempty R] [DecidableEq (Fin q)]
  ...
  set acc := qAcceptProb A x accept; set N := (Fintype.card R : ℝ≥0∞)
  set Q := (q : ℝ≥0∞)
  set accI := fun I : Fin q =>
    acceptProb (guessReduction (R := R) A I) x (acceptAtGuess accept I)
  have hQ_ne : Q ≠ 0 := Nat.cast_ne_zero.mpr (Nat.pos_iff_ne_zero.mp hq)
  have hQ_ne_top : Q ≠ ⊤ := ENNReal.natCast_ne_top q
  rw [qForkSuccProb, ge_iff_le, tsub_le_iff_right]
  -- Partition: ∑ accI = acc
  have hp : ∑ I : Fin q, accI I = acc := sum_acceptAtGuess_eq A x accept
  -- Fork bound per I (rearranged): accI² ≤ forkI + accI/N
  have hfb : ∀ I : Fin q, accI I ^ 2 ≤
      forkSuccProb (guessReduction (R := R) A I) x (acceptAtGuess accept I) + accI I / N :=
    fun I => tsub_le_iff_right.mp (fork_success_ge _ x _)
  -- Sum: ∑ accI² ≤ ∑ fork + acc/N
  have hsf : ∑ I, accI I ^ 2 ≤
      (∑ I, forkSuccProb (guessReduction (R := R) A I) x (acceptAtGuess accept I)) + acc / N := by
    calc ∑ I, accI I ^ 2
        ≤ ∑ I, (forkSuccProb (guessReduction (R := R) A I) x (acceptAtGuess accept I) +
          accI I / N) :=
          Finset.sum_le_sum fun I _ => hfb I
      _ = (∑ I, forkSuccProb (guessReduction (R := R) A I) x (acceptAtGuess accept I)) +
          ∑ I, accI I / N := Finset.sum_add_distrib
      _ ≤ _ := by
          gcongr
          exact le_of_eq (by
            calc ∑ I, accI I / N
                = (∑ I, accI I) / N := by simp only [div_eq_mul_inv, Finset.sum_mul]
              _ = acc / N := by rw [hp])
  -- Cauchy-Schwarz: acc²/Q ≤ ∑ accI²
  have hcs : acc ^ 2 / Q ≤ ∑ I, accI I ^ 2 := by
    rw [← hp, ENNReal.div_le_iff (by positivity) hQ_ne_top, mul_comm]
    set S := ∑ I, accI I
    have hSS : S * S = ∑ i : Fin q, ∑ j : Fin q, accI i * accI j := by
      change (∑ i, accI i) * (∑ j, accI j) = ∑ i, ∑ j, accI i * accI j
      rw [Finset.sum_mul]; congr 1; funext; rw [Finset.mul_sum]
    rw [sq S, hSS]
    suffices h2mul : 2 * ∑ i : Fin q, ∑ j : Fin q, accI i * accI j ≤
        2 * (↑q * ∑ j, accI j ^ 2) by
      exact (ENNReal.mul_le_mul_iff_right (by norm_num) (by norm_num)).mp h2mul
    calc 2 * ∑ i, ∑ j, accI i * accI j
        = ∑ i, ∑ j, 2 * (accI i * accI j) := by
          rw [Finset.mul_sum]; congr 1; funext i; rw [Finset.mul_sum]
      _ ≤ ∑ i, ∑ j, (accI i ^ 2 + accI j ^ 2) := by
          apply Finset.sum_le_sum; intro i _
          apply Finset.sum_le_sum; intro j _
          rw [← mul_assoc]; exact ennreal_two_mul_le_add_sq _ _
      _ = ∑ i, (↑q * accI i ^ 2 + ∑ j, accI j ^ 2) := by
          congr 1; funext i; rw [Finset.sum_add_distrib]; congr 1
          simp [Finset.sum_const, nsmul_eq_mul]
      _ = ↑q * ∑ i, accI i ^ 2 + ↑q * ∑ j, accI j ^ 2 := by
          rw [Finset.sum_add_distrib]; congr 1
          · exact (Finset.mul_sum ..).symm
          · simp [Finset.sum_const, nsmul_eq_mul]
      _ = 2 * (↑q * ∑ j, accI j ^ 2) := by ring
  -- Chain: acc²/Q ≤ ∑ accI² ≤ ∑ fork + acc/N
  calc acc ^ 2 / Q ≤ ∑ I, accI I ^ 2 := hcs
    _ ≤ _ := hsf
